...
### Regression model of all measurements

```{r}
cardiac  %>% spread(meth,y,fill=NA) %>%ggplot(aes(x=IC,y=RV)) + 
  geom_point() + 
...
```{r}
cardiac.lm.1 <- cardiac  %>% spread(meth,y,fill=NA) 
confint(lm(IC ~RV,data=cardiac.lm.1))
```

```{r}
cardiac.lm.2 <-cardiac %>% group_by(meth,item) %>% summarize(ybar =mean(y,na.rm=TRUE)) %>% spread(meth,ybar)
cardiac.lm.2 %>%ggplot(aes(x=IC,y=RV)) + 
  geom_point() + 
  geom_smooth(method="lm") + 
  geom_abline(intercept = 0,slope=1)
```
...
confint(lm(IC ~RV,data=cardiac.lm.2))
```
...
comparison studies. Statistical Methods in Medical Research, 8:136-160, 1999. 
* Originally supplied
