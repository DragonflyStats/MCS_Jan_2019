...
```
```{r}
Enzyme.lm <- Enzyme %>% spread(meth,y)
confint(lm(SucHom~SucPel,data=Enzyme.lm))
```
```{r}
Enzyme <- Meth( Enzyme )
summary( Enzyme )
...
dat=Enzyme %>% filter(meth!="SucHom")

Deming(Enzyme.lm$SucPel,Enzyme.lm$Alkphos,boot=TRUE,keep.boot = TRUE)
```
